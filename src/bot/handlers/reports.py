...
async def cmd_today(message: Message) -> None:
    ...
    hours = compute_period_hours(closed, today, today, tz)
...
async def cmd_week(message: Message) -> None:
    ...
    closed = [s for s in shifts if s.end_at is not None]
    hours = compute_period_hours(closed, start_of_week, today, tz)
...
async def cmd_month(message: Message) -> None:
    ...
    closed = [s for s in shifts if s.end_at is not None]
    hours = compute_period_hours(closed, start_of_month, today, tz)
